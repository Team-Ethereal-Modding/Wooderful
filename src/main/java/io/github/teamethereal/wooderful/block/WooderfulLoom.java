...
public class WooderfulLoom extends LoomBlock {

	public WooderfulLoom(Settings settings) {
		super(settings);
	}
	...
	public void addStacksForDisplay(ItemGroup group, DefaultedList<ItemStack> list) {
		if(ItemStackUtils.isInGroup(this.asItem(), group)) {
			if (FabricLoader.getInstance().isModLoaded("minecraft")) {
				int targetIndex = ItemStackUtils.findIndexOfItem(Blocks.LOOM.asItem(), list);
				if(targetIndex != -1) {
