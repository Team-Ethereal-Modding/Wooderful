...
public class WooderfulFletchingTable extends FletchingTableBlock {

	public WooderfulFletchingTable(Settings settings) {
		super(settings);
	}
	...
	public void addStacksForDisplay(ItemGroup group, DefaultedList<ItemStack> list) {
		if(ItemStackUtils.isInGroup(this.asItem(), group)) {
			if (FabricLoader.getInstance().isModLoaded("minecraft")) {
				int targetIndex = ItemStackUtils.findIndexOfItem(Blocks.FLETCHING_TABLE.asItem(), list);
				if(targetIndex != -1) {
